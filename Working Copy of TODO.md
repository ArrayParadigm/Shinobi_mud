...
- [ ] Update fight system. attack speed should be based on Dex and agi, Maybe "base" speed is 3, and skills, jutsu, and attributes will affect it.. so, 10 dex, and 10 agi will be a "100%" or basically 1 attack a second, then up to 5 per second. . See below:
                    ...
                    stat_avg = (DEX + AGI) / 2
                    stat_progress = (stat_avg - MIN_STAT) / (MAX_STAT - MIN_STAT)

                    APS = MIN_APS + (MAX_APS - MIN_APS) * stat_progress
                    APS = clamp(APS, MIN_APS, MAX_APS)