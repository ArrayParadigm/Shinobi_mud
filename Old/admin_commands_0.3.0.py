...
def copyover(protocol, players_in_rooms=None):
    """Perform a soft reboot (copyover) of the MUD."""
    protocol.sendLine(b"Initiating copyover... Please hold on.")
    logging.info(f"User {protocol.username} initiated copyover.")

    # Save state if necessary (optional at this point)
    # save_state()

    try:
        args = sys.argv[:]
        args.insert(0, sys.executable)  # Prepend the Python executable path
        os.execv(sys.executable, args)  # Replace the current process
