...
COMMAND_REGISTRY = {**general_commands, **admin_commands, **social_commands}
    ...
    **{cmd: lambda *args, func=func: func(*args, cursor) for cmd, func in general_commands.items() if cmd == "status"},
    **{cmd: func for cmd, func in general_commands.items() if cmd != "status"},
...
class NinjaMUDProtocol(basic.LineReceiver):
    ...
    def register_password(self, line):
        ...
        self.character_creation_data['password'] = line.decode('utf-8').strip()
